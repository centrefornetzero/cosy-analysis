...
df <- df %>% mutate(temperature = round(daily_avg_heating_degree))
reg <- feols(consumption_hh ~ i(is_hp_installed, ref=0) +
...
# Extract FE predictions
fe_account <- fes$account_id[as.character(df$account_id)]
fe_date <- fes$date[as.character(df$date)]
fe_hdd <- fes$temperature[as.character((df$temperature))]

# Extract interaction term matrix
X <- model.matrix(reg)
interaction_cols <- grep("\\hp_engineer", colnames(X), value = TRUE)
interaction_matrix <- X[, interaction_cols, drop = FALSE]
interaction_coefs <- coef(reg)[interaction_cols]
interaction_pred <- as.numeric(interaction_matrix %*% interaction_coefs)

# --- ATE Term ---
ate_col <- grep("is_hp_installed::1$", colnames(X), value = TRUE)
ate_matrix <- X[, ate_col, drop = FALSE]
ate_coef <- coef(reg)[ate_col]
ate_pred <- as.numeric(ate_matrix * ate_coef)

# Total variance in outcome
var_total <- var(df$consumption_hh, na.rm = TRUE)

# Component variances
var_account <- var(fe_account, na.rm = TRUE)
var_date <- var(fe_date, na.rm = TRUE)
var_temperature <- var(fe_hdd, na.rm = TRUE)
var_interaction <- var(interaction_pred, na.rm = TRUE)
var_ate <- var(ate_pred, na.rm = TRUE)

# Shares
share_ate <- var_ate / var_total
share_account <- var_account / var_total
share_date <- var_date / var_total
share_interaction <- var_interaction / var_total
share_temperarure <- var_temperature / var_total

# Create results table
var_decomp <- data.frame(
  Component = c("Account FE", "Date FE", "HDD FE",
                "ATE of Installation", 
                "Engineer-Specific Effect"),
  Share = c(share_account, share_date, share_temperarure, share_ate, share_interaction)
)
...
# Merge in time fixed effects if needed (skip if you're not residualizing)
# Assuming you extracted date fixed effects from a separate model
# fes$date is a named vector of date fixed effects
df2 <- df %>%
  inner_join(data.frame(date = as.Date(names(fes$date)), time_fe = fes$date))%>%
  inner_join(data.frame(temperature = as.numeric(names(fes$temperature)), 
                        temp_fe = fes$temperature))

# Residualize outcome by subtracting time fixed effects
df2$consumption_hh_time <- df2$consumption_hh - df2$time_fe - df2$temp_fe

# Estimate fixed effects model with treatment + interaction
reg_sub <- felm(
  consumption_hh_time ~ is_hp_installed +
    is_hp_installed:hp_engineer | 
    account_id,  # already removed date FE via residualization
  data = df2,
  cmethod = "reghdfe"
)

# 4. Extract FE covariance matrix
fe_sub <- getfe(reg_sub, se = TRUE)
covar <- fevcov(reg_sub, fe_sub)
covar_biased <- covar + attr(covar, "bias")
attr(covar_biased, "bias") <- NULL

# 5. Variance of the residualized outcome
total_var <- var(df2$consumption_hh_time, na.rm = TRUE)

# 6. Extract model matrix and coefficients
X <- model.matrix(reg_sub)
coefs <- coef(reg_sub)

# 7. Compute predicted values for:
#    - is_hp_installed main effect
#    - interaction term (is_hp_installed × hp_engineer)

# Main treatment effect (is_hp_installed)
main_cols <- grep("^is_hp_installed$", colnames(X))
main_pred <- X[, main_cols, drop = FALSE] %*% coefs[main_cols]

# Interaction terms (is_hp_installed:hp_engineer)
main_pred <- grep("^is_hp_installed:hp_engineer", colnames(X))
interact_pred <- X[, interact_cols, drop = FALSE] %*% coefs[interact_cols]

# 8. Variances
var_main <- var(as.numeric(main_pred), na.rm = TRUE)
var_interact <- var(as.numeric(interact_pred), na.rm = TRUE)

# 9. Variance shares
share_main <- var_main / total_var
share_interact <- var_interact / total_var

# 10. Combine with FE variance shares
var_share_fe <- covar / total_var
attr(var_share_fe, "bias") <- NULL

# Combine all into a summary data frame
var_decomp <- data.frame(
  Component = c("Account FE", "Main Treatment Effect", "Treatment × Engineer"),
  Variance_Share = c(diag(var_share_fe), share_main, share_interact)
)
...
cat("\n===== Variance Share (including treatment terms) =====\n")
print(var_decomp)
```

```{r account-interaction-covariance}

# 1. Extract account fixed effects
fes <- getfe(reg_sub, se = TRUE)
account_fe_vec <- fes[fes$fe == "account_id", c("idx", "effect")]
colnames(account_fe_vec) <- c("account_id", "account_fe")

# Convert account_id to the same type as in df2 (likely character)
account_fe_vec$account_id <- as.character(account_fe_vec$account_id)
df2$account_id <- as.character(df2$account_id)

# Merge account fixed effects back into the main data
df2 <- merge(df2, account_fe_vec, by = "account_id")

# 2. Recalculate interaction predicted values (if not already)
X <- model.matrix(reg_sub)
coefs <- coef(reg_sub)

interact_cols <- grep("^is_hp_installed:hp_engineer", colnames(X))
interact_pred <- as.numeric(X[, interact_cols, drop = FALSE] %*% coefs[interact_cols])
...
# 4. Optionally: standardize to correlation
cor_account_interaction <- cor(df2$account_fe, df2$interaction_pred, use = "complete.obs")

# Print the result
cat("\n===== Covariance (Account FE × Interaction Term) =====\n")
print(cov_account_interaction)

cat("\n===== Correlation (Account FE × Interaction Term) =====\n")
print(cor_account_interaction)
...
# # 2. Merge account FE back to df
# df2$account_id <- as.character(df2$account_id)
# df2 <- merge(df2, account_fe, by = "account_id")

# 4. Compute average account FE per engineer (only among treated)
engineer_house_fe <- df2 %>%
  group_by(hp_engineer) %>%
  summarise(avg_account_fe = mean(account_fe, na.rm = TRUE))

# 5. Extract interaction term coefficients
interaction_coefs <- coef(summary(reg_sub)) %>%
  ...
  mutate(hp_engineer = gsub("is_hp_installed:hp_engineer", "", term),
         hp_engineer = gsub("^\\(", "", gsub("\\)$", "", hp_engineer)))

# 6. Merge average account FE and interaction effect
plot_data <- left_join(engineer_house_fe, interaction_coefs, by = "hp_engineer")

# 7. Plot
ggplot(plot_data, aes(x = avg_account_fe, y = Estimate), color = hp_engineer) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "steelblue") +
  labs(
    x = "Average Account Fixed Effect",
    y = "Engineer-Specific Treatment Effect",
    title = "Relationship Between Household FE and Engineer Treatment Effects"
  ) +
  theme_minimal()
