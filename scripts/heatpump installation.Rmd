...
# If HonestDid cannot be installed from CRAN, install it from GitHub
...
# run on a subsample for faster processing
if (random_subsample) {
  set.seed(123)
...
apply_twfe_models <- function(data, filename_prefix) {
  ...
  m1 <- feols(elec_consumption ~ i(is_hp_installed) | account_id + avg_heating_degree + settlement_week, 
              data = data %>% filter(settlement_week < "2024-06-03"), cluster = ~ account_id)
