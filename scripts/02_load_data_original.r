...
if(!file.exists(file.path(datapath, "scratch/aggregated_data_old.RDS"))) {
    print('hello')
  ...
  aggregated_data_old <- 
    ...
    fread(file.path(datapath, "input/cosy_-_cosy_electricity_reading_part_2_2024_07_26.csv")) %>%
...
## HERE!!!!!
  ...
  aggregated_data_old <- readRDS(file.path(datapath, "scratch/aggregated_data_old.RDS")) 
}

# Run on a subsample of the data for faster processing
if (random_subsample) {
  set.seed(123)
  sampled_accounts <- sample(unique(aggregated_data_old$account_id), 1000)
  aggregated_data_old <- aggregated_data_old %>% 
